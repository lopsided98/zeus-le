...
#define Q_DEFINE(lp, up, tp, b, w, f)                                         \
    ...
    static const Q_TYPE(lp, w, f) Q_NAME(up, w, f, ONE) = (Q_TYPE(lp, w, f))1 \
                                                          << f;               \
                                                                              \
    static Q_TYPE(lp, w, f) Q_NAME(lp, w, f, from_int)(tp##w##_t val) {       \
        return (Q_TYPE(lp, w, f))val << f;                                    \
    }                                                                         \
                                                                              \
    static tp##w##_t Q_NAME(lp, w, f, whole)(Q_TYPE(lp, w, f) val) {          \
        return val >> f;                                                      \
    }                                                                         \
                                                                              \
    static float Q_NAME(lp, w, f, to_float)(Q_TYPE(lp, w, f) val) {           \
        return (float)val / Q_NAME(up, w, f, ONE);                            \
    }                                                                         \
                                                                              \
    static double Q_NAME(lp, w, f, to_double)(Q_TYPE(lp, w, f) val) {         \
